...
class _CreateAccountScreenState extends State<CreateAccountScreen>
  ...
  static const String _otpServerUrl =
  ...
  Future<Map<String, dynamic>> _createFirebaseAccount(
    ...
    try {
      ...
      switch (e.code) {
        ...
        default:
          errorMessage =
              'An error occurred during account creation: ${e.message}';
      }
      return {
        'success': false,
        'error': errorMessage,
  ...
  Future<Map<String, dynamic>> _saveUserDataToFirestore(
    ...
    try {
      await _firestore.collection('users').doc(user.uid).set({
        ...
        'isActive': true,
        'lastLoginAt': null,
        'profileCompleted': false, // Added to track profile completion status
        'preferences': {
          'notifications': true,
          'darkMode': false,
          'language': 'en',
  ...
  Future<Map<String, dynamic>> _sendOTPEmail(String email, String name) async {
    try {
      debugPrint('Sending OTP request to: $_otpServerUrl/send-otp');
      debugPrint('Request body: {email: $email, name: $name}');
      ...
      if (response.statusCode == 200) {
        ...
        if (!result.containsKey('success')) {
          result['success'] =
              true; // Assume success if no explicit success field
        }
        return result;
      } else {
      ...
      return {
        'success': false,
        'error':
            'Network error: Unable to connect to email service. Please check your internet connection and try again.',
      };
    }
  }
  ...
  void _onSignUpPressed() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        Map<String, dynamic> firebaseResult = await _createFirebaseAccount(
              ...
              SnackBar(
                content: Column(
                  ...
                  children: [
                    ...
                    Text(
                      firebaseResult['error'] ?? 'Unknown error occurred',
                      style: GoogleFonts.albertSans(
        ...
        Map<String, dynamic> firestoreResult = await _saveUserDataToFirestore(
          firebaseUser,
          _nameController.text.trim(),
        );

        if (!firestoreResult['success']) {
          // If Firestore save fails, we should delete the Firebase user
              ...
              SnackBar(
                content: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      firestoreResult['error'] ?? 'Unknown error occurred',
                      style: GoogleFonts.albertSans(
                        fontWeight: FontWeight.w400,
                        fontSize: 12,
                      ),
        ...
        // Step 3: Send OTP email using HTTP request
        Map<String, dynamic> otpResult = await _sendOTPEmail(
          _emailController.text.trim(),
          _nameController.text.trim(),
        );

        // Debug: Print the OTP result to see what we're getting
        ...
        if (mounted) {
          ...
          if (otpResult['success'] == true ||
                ...
                PageRouteBuilder(
                  pageBuilder: (context, animation, secondaryAnimation) =>
                    ...
                    sentOTP: otpResult['otp'] ??
                        otpResult['code'] ??
                        '', // Handle different OTP field names
            ...
            // If OTP sending fails, we should clean up the created Firebase user and Firestore data
            try {
              // Delete Firestore document first (while user is still authenticated)
              await _firestore
                  .collection('users')
                  .doc(firebaseUser.uid)
                  .delete();
              // Then delete Firebase user (this removes authentication context)
              await firebaseUser.delete();
            } catch (e) {
              debugPrint('Error cleaning up after OTP failure: $e');
              // Try to delete Firebase user even if Firestore deletion failed
              try {
                await firebaseUser.delete();
              } catch (userDeleteError) {
                debugPrint('Error deleting Firebase user: $userDeleteError');
              }
            }
              ...
              SnackBar(
                content: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      otpResult['error'] ?? 'Unknown error occurred',
                      style: GoogleFonts.albertSans(
                        fontWeight: FontWeight.w400,
                        fontSize: 12,
                      ),
      ...
      PageRouteBuilder(
        ...
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          return SlideTransition(
            position: Tween<Offset>(
              begin:
                  const Offset(-1.0, 0.0), // Slide from left (back navigation)
              end: Offset.zero,
            ).animate(CurvedAnimation(
              parent: animation,
              curve: Curves.easeOutCubic,
